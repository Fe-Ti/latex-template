...
Начало секции приложений к отчёту ознаменовывается командой `\appendix`. Она выставляет счётчики в 0 и переопределяет команды `\thefigure`, `\thetable`, `\theequation` и `\thelstlisting` так, чтобы оформление соответствовало ГОСТу.
...
Использовать `\section{...}` здесь НЕ НАДО.
